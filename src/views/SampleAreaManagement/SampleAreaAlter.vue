<template>
    ...
    <template #content>
      <span class="text-large font-600 mr-3"> 样区修改 </span>
    </template>
  </el-page-header>
  <el-button @click="changemap">切换地图模式</el-button>
    ...
    <el-select placeholder="请选择防控区" style="width: 60%;" v-model="fkarea" clearable @change="oldline">
    <el-option
...
let row=router.currentRoute.value.query
 ...
 const habitats=[{label:'全部',value:0},{label:'公园',value:1},{label:'景区',value:2},{label:'广场',value:3},{label:'园区',value:4},{label:'厂区',value:5},{label:'学校',value:6},{label:'村庄',value:7},{label:'社区',value:8},{label:'种养基地',value:9},{label:'企事单位',value:10}]
  ...
  let mouseTool:any
    // 切换地图模式
    const changemap=()=>{
    router.push({path:'/sampleareamanagement/sampleareanormalalter',query:row})
  }
  //  返回
  const goBack=()=>{
  ...
  const init = (e:any) => {
    ...
    oldline();
    yqoldlines();
    (fkarea.value as any)=(row.aid as any)*1;
    (fkname.value as any)=row.pdname;
    habitat.value=(row.sgtype as any)*1;
    (imageUrl.value as any)=row.imgurl;
    fileList=[]
...
endLoading()
  ...
  let oldlines:any=reactive([])
  let alloldlines:any=[]
  // 绘制防控区边界
const oldline=()=>{
  map.remove(oldlines);
  map.remove(alloldlines);
  oldlines=[];
    alloldlines=[];
  axios.get('/antmonitor/unit/pjas?pjid='+id).then(res=>{
        let lines:any=[]
        res.data.data.forEach((i:any) => {
          if(fkarea.value==i.aid)
          {
            (i.lls as any).split(';').forEach((e:any) => {
              let lng=e.split(',')[0]*1
            let lat=e.split(',')[1]*1
           let lnglat:any[]=[lng,lat];
           (oldlines as any).push(lnglat)
            });
    ...
    let bezierCurve = new AMap.Polygon({
        ...
        // 将需要绘制的线条存储到数组中
        alloldlines.push(bezierCurve)
        }
  map.add(alloldlines);
  (map as any).setFitView(alloldlines);
          }
        })
    })
...
const yqoldlines=()=>{
  yqlines=[];
  yqalllines=[];
  (row.lnglats as any).split(';').forEach((e:any) => {
              let lng=e.split(',')[0]*1
            let lat=e.split(',')[1]*1
           let lnglat:any[]=[lng,lat];
           (yqlines as any).push(lnglat)
    })
    for(let i=0;i<yqlines.length;i++){
    let bezierCurve = new AMap.Polygon({
            path:yqlines,
            // strokeDasharray: [5, 5],
            fillColor: 'transparent',
...
const handleChange=(file:any)=>{
  ...
  const save=()=>{
    ...
    let polnglats=row.lnglats
        param.append('id',(row.id as any))
        param.append('aid',fkarea.value)
        param.append('lnglats',(polnglats as any))
        param.append('carea',(realVal as any))
       ...
       if(fileList[0]){
        param.append('file',(fileList[0] as any).raw)
         ...
         // 存储边界
      axios({url: '/antmonitor/unit/udpdt',
              data: param,
              method: 'post',
              headers: { 
