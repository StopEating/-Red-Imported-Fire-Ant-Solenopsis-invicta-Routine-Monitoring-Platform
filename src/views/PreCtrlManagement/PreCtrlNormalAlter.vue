    ...
    if(polnglat!==''&&polnglat!==undefined){
      ...
      if(fkname.value==''){
        ...
        type: 'warning',
         ...
         // 存储边界
      axios.post('/antmonitor/unit/udpja?id='+row.id+'&lnglats='+polnglats+'&carea='+realVal+'&aname='+fkname.value+'&lnglat='+center.value+'&sgtype='+habitat.value+'&createuser='+store.state.user.username).then(res=>{
        console.log(res)
        reset()
        ElMessage({
        message: '管理防控区成功！',
