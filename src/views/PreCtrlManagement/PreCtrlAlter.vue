<template>
  ...
  <template #content>
    <span class="text-large font-600 mr-3"> 防控区修改 </span>
  </template>
</el-page-header>
<el-button @click="changemap">切换地图模式</el-button>
  ...
  <div>  
    <span style="width:30%;display: inline-block">防控区名称：</span>
    <el-input
    type="textarea"
    autosize
    placeholder="请输入内容"
    v-model="fkname"
    clearable style="width:65%;vertical-align: middle;">
  </el-input>
</div>
 <div style="margin-top:5px"> 
  <span style="width:30%;display: inline-block;">生境:</span>
  <el-select v-model="habitat" placeholder="请选择" style="width:65%;">
  <el-option
...
let oldcenter=router.currentRoute.value.query.lnglats
let fkname=ref(row.aname)
...
let habitat=ref((row.sgtype as any)*1)
const zoom = ref(18);
...
let nocontents=reactive([])
 interface options{
  ...
  let mouseTool:any
    // 切换地图模式
    const changemap=()=>{
    router.push({path:'/prectrlmanagement/prectrlnormalalter',query:row})
  }
//  返回
const goBack=()=>{
...
map = e;
  // map.setZoomAndCenter(zoom.value, [center.value]); //同时设置地图层级与中心点
  ...
  if(polnglat!==''&&polnglat!==undefined){
    ...
    if(fkname.value==''){
      ...
      type: 'warning',
       ...
       // 存储边界
    axios.post('/antmonitor/unit/udpja?id='+row.id+'&lnglats='+polnglats+'&carea='+realVal+'&aname='+fkname.value+'&lnglat='+center.value+'&sgtype='+habitat.value).then(res=>{
      console.log(res)
      reset()
      ElMessage({
      message: '管理防控区成功！',
